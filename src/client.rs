...
use sysinfo::{Pid, PidExt, Process, ProcessExt, ProcessStatus, System, SystemExt};
use url::Url;

use crate::{
    daemon::{self, Daemon, DaemonError, DaemonRequest, DaemonResponse, DaemonStatus},
    xdg,
};
...
impl Display for ClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ClientError::PrepareRequest(e) => {
                write!(f, "Couldn't prepare HTTP request to daemon: {}", e)
            }
            ClientError::ExecuteRequest(e) => {
                write!(f, "Couldn't send HTTP request to daemon: {}", e)
            }
            ClientError::ParseResponse(e) => {
                write!(f, "Couldn't parse JSON response from daemon: {}", e)
            }
            ClientError::UnexpectedResponse(req, resp) => {
                write!(f, "Unexpected response from {:?}: {:?}", req, resp)
            }
            ...
            ClientError::AuthURIMissingCode => {
                write!(f, "Authorization URI was missing code param")
            }
            ClientError::AuthURIMissingState => {
                write!(f, "Authorization URI was missing state param")
            }
            ClientError::AuthURIMissingIntent => {
                write!(f, "Authorization URI was missing intent param")
            }
            ClientError::AuthURIIntentInvalid(s) => write!(
                f,
                "Authorization URI intent param was an invalid value: {}",
                s
            ),
            ClientError::DaemonCrashed => {
                write!(f, "Spawned daemon, but then it crashed. Check daemon logs.")
            }
        }
    }
}
...
impl Client {
    ...
    fn execute_request(&self, daemon_request: &DaemonRequest) -> ClientResult<DaemonResponse> {
        let request = daemon_request
            .to_reqwest_request(&self.url_base, &self.http_client)
            .map_err(ClientError::PrepareRequest)?;
        let response = self
            .http_client
            .execute(request)
            .map_err(ClientError::ExecuteRequest)?;
        let daemon_response: DaemonResponse =
            response.json().map_err(ClientError::ParseResponse)?;
        Ok(daemon_response)
    }

    pub fn authorize(&self, uri_arg_str: &str) -> ClientResult<DaemonResponse> {
        let params: Vec<(&str, &str)> = uri_arg_str
            .split(",")
            .map(|p| p.split_once("="))
            .filter(|po| po.is_some())
            .map(|po| po.unwrap())
            .collect();

        let auth_code_str = params
            .iter()
            .find(|(k, _)| *k == "code")
            .ok_or(ClientError::AuthURIMissingCode)?
            .1;
        let state_str = params
            .iter()
            .find(|(k, _)| *k == "state")
            .ok_or(ClientError::AuthURIMissingState)?
            .1;
        let intent = params
            .iter()
            .find(|(k, _)| *k == "intent")
            .ok_or(ClientError::AuthURIMissingIntent)?
            .1
            .to_string();
    ...
    fn launch_and_print_result(&self, display_name: Option<&str>) {
        let daemon_request = DaemonRequest::Launch {
        ...
        match response {
            ...
            DaemonResponse::ErrorStr(s) => {
                tracing::warn!("Couldn't launch game due to error: {}", s)
            }
    ...
    fn open_url_and_print_err(&self, url_str: &str) {
        let open_result: Result<std::process::Child, String> = Url::parse(&url_str)
    ...
    fn fetch_auth_url(&self, deauth: Option<bool>) -> ClientResult<String> {
        let request = DaemonRequest::AuthorizationURLRequest {
            deauth: deauth.unwrap_or(false),
        };
                ...
                tracing::subscriber::with_default(log_collector, || {
                    ...
                    match Daemon::new(listen_address) {
                        Ok(mut d) => {
                            let r = d
                                .run()
                                .map(|_| DaemonError::HTTPServerClosed)
                                .unwrap_or_else(|e| e);
        ...
        match choices.len() {
            ...
            _ => {
                ...
                if std::io::stdin().read_line(&mut line_buf).is_ok() {
                    ...
                    let char_by_name = choices.iter().find(|c| c.eq_ignore_ascii_case(&line));
                    match char_by_name {
                        ...
                        None => {
                            if let Ok(idx) = usize::from_str_radix(&line, 10) {
                                ...
                                match choices.get(idx - 1) {
                                    ...
                                    Some(char_by_idx) => {
                                        self.launch_and_print_result(Some(&char_by_idx))
                                    }
