...
use crate::{
    daemon::{
        consent_client::ConsentClientSession, game_session::AccountSession,
        launcher_client::LauncherClientSession,
    },
    xdg::XDGCredsState,
};
...
fn main() -> ClientResult<()> {
    ...
    if let Some(jagex_params) = std::env::args()
        .nth(1)
        .and_then(|s| s.strip_prefix("jagex:").map(|s| s.to_string()))
    {
        ...
        if args.clear_creds {
            if let Ok(path) = LauncherClientSession::ensure_creds_file_path() {
                if path.exists() {
                    std::fs::remove_file(path)
                        .expect("Couldn't remove launcher client credentials file");
            ...
            if let Ok(path) = ConsentClientSession::ensure_creds_file_path() {
                if path.exists() {
                    std::fs::remove_file(path)
                        .expect("Couldn't remove consent client credentials file");
            ...
            if let Ok(path) = AccountSession::ensure_creds_file_path() {
                if path.exists() {
                    std::fs::remove_file(path)
                        .expect("Couldn't remove account session credentials file");
